...
I tailed the journal expecting some smoking gun. What I got was the same line, on repeat:
...
- **I think the `--connect` to a known bridge node is not optional on mainnet today.** The Utreexo network is too small for organic peer discovery.
