...
- **Address Format**:
  Bech32m-encoded com chaves de scan ($B_{\text{scan}}$) e spend ($B_{\text{spend}}$);
  labels fazem tweak:
  $$B_m = B_{\text{spend}} + \bigl(\operatorname{hash}(B_{\text{scan}} \Vert m)\bigr)\,G.$$

- **Sender Process**:
  Somar chaves privadas de entrada:
  $$a = \sum_i a_i.$$
  Calcular `input_hash` a partir do menor outpoint:
  $$h_{\text{in}} = \operatorname{hash}(\min(\text{outpoints})).$$
  Derivar segredo ECDH:
  $$\text{shared\_secret} = h_{\text{in}} \cdot a \cdot B_{\text{scan}}.$$
  Tweak para saídas:
  $$t_k = \operatorname{hash}(\text{shared\_secret} \Vert k), \qquad
  P = B_m + t_k \, G.$$
